...
from .scanner import raw_read, raw_patch
...
def apply_set(data: bytes, key: str, val: str) -> bytes:
    ...
    print(f'[✓] {key} = {disp} ({"patched in-place" if found else "appended new block"})')
    return new_data
...
def export_json(data: bytes, path: Path):
    ...
    for block_id, (name, fmt) in sorted(DBI_SCHEMA.items()):
        ...
        if fmt == 'ba':
            result[name] = {'_type': 'ba', 'data': base64.b64encode(value).decode() if value else None}
