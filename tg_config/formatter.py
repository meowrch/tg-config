...
def fmt_value(name: str, fmt: str, value) -> str:
    ...
    if fmt == 'i32':
        ...
        elif name in ('AutoStart', 'StartMinimized', 'SoundNotify', 'DesktopNotify',
...
def dump_all(data: bytes, verbose: bool = False):
    ...
    for block_id, (name, fmt) in sorted(DBI_SCHEMA.items()):
        ...
        print(f' {CYAN}0x{block_id:04X}{RESET} {BOLD}{name:<35}{RESET} {GREEN}{val_str}{RESET}')
        if verbose:
...
def dump_app_settings(blob: bytes, verbose: bool = False):
    ...
    while p + 4 <= len(blob):
        ...
        try:
            if fmt == 'i32':
                value = struct.unpack_from('>i', blob, p)[0]; p += 4
            elif fmt == 'str':
...
def probe_block(data: bytes, offset: int):
    ...
    candidates = [
        ('i32 (4B)', 4), ('i64 (8B)', 8),
        ('12B', 12), ('16B', 16), ('20B', 20),
        ('24B', 24), ('28B', 28), ('32B', 32),
        ('36B', 36), ('40B', 40),
    ]
    vp = offset + 4
    if vp + 4 <= len(data):
